...
public:
	...
	InputText(int x0, int y0, unsigned int maxlen0, int font = -1) : Obj(x0, y0), CancelValidFlag{false}, SingleCharOnlyFlag{false}, NumCharOnlyFlag{false}, maxlen{maxlen0}, fontHandle{font}
	{
		...
		if (inputHandle == -1)
			throw new std::runtime_error("テキスト入力オブジェクト作成失敗");
	...
	InputText(
		...
		int font = -1) : Obj(x0, y0), CancelValidFlag{CancelValidFlag0}, SingleCharOnlyFlag{SingleCharOnlyFlag0}, NumCharOnlyFlag{NumCharOnlyFlag0}, maxlen{maxlen0}, fontHandle{font}
	{
		__textptr = new char[static_cast<ULL>(maxlen) + 1ull];
